...
export function printServiceMaintenanceContract(contract: Contract): boolean {
  const popup = window.open("", "_blank", "width=950,height=850");
  if (!popup) return false;
  const customer = contract.customer || contract.manager;
  const address = contract.address;
  // مبالغ قرارداد در برنامه به ریال نگهداری می‌شوند؛ متن چاپی مبلغ را به تومان می‌خواهد.
  const amount = contract.monthlyServiceFee ? Math.round(Number(contract.monthlyServiceFee) / 10).toLocaleString("fa-IR") : "........................................";
  ...
  <p>احتراماً پیرو درخواست حضرتعالی جهت سرویس و نگهداری یک دستگاه آسانسور کششی به آدرس: <span class="field">${line(address)}</span> به‌عنوان کارفرما و آقای <b>محسن امامی برسری</b> به نمایندگی شرکت بنیان نوین گستر آسمان‌سرا به‌عنوان پیمانکار، این قرارداد به شرح ذیل منعقد می‌گردد:</p>
  <p><span class="subject">موضوع قرارداد:</span> سرویس و نگهداری و رفع عیب .......... دستگاه آسانسور .......... توقف با درب .......... اتوماتیک با ظرفیت .......... نفر معادل .......... کیلوگرم.</p>
  <p><span class="subject">مدت قرارداد:</span> به مدت یک سال از تاریخ <span class="field">${line(contract.start, "....................")}</span> لغایت <span class="field">${line(contract.end, "....................")}</span> خواهد بود. بازه سرویس‌دهی به دلیل خرابی‌های پیش‌بینی‌نشده ۱ تا ۷ روز قابل جابه‌جایی می‌باشد. صرفاً فقط در تاریخ مذکور، سرویس انجام نمی‌شود.</p>
